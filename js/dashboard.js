...
// Raw data (semua titik) & view data (subset sesuai filter)
const store = {
  raw: { ts: [], t: [], h: [] },
  view: { labels: [], t: [], h: [] }
};
...
// Chart & filter
let chart = null;
let rangeMinutes = 15;             // 15m default, "all" = semua
const MAX_POINTS = 1000;           // hard cap tampilan

// Offline ESP detector
let lastSeenTs = 0;
const OFFLINE_MS = 15000;          // 15 detik tanpa heartbeat => offline
let offlineTimer = null;

// ===== Init =====
document.addEventListener('DOMContentLoaded', () => {
  // Hilangkan spinner setelah render
  setTimeout(() => {
    const ov = document.getElementById('loading-overlay');
    if (ov) ov.classList.add('hidden');
  }, 1200);

  // Browser online/offline (opsional)
  window.addEventListener('offline', () => showToast('Koneksi Putus', 'Periksa internet Anda', 'warning'));
  window.addEventListener('online',  () => showToast('Online', 'Koneksi internet kembali', 'success'));
...
function setupFirebase(){
  ...
  // Sensor stream (dipanggil tiap ESP update)
  ...
  lastSeenRef.on('value', (snap) => {
    lastSeenTs = Number(snap.val() || 0);
    evaluateDeviceOnline();
  });

  // Evaluasi offline tiap 5 dtk
  if (offlineTimer) clearInterval(offlineTimer);
  offlineTimer = setInterval(evaluateDeviceOnline, 5000);
}

function evaluateDeviceOnline(){
  const now = Date.now();
  const online = (lastSeenTs > 0) && (now - lastSeenTs <= OFFLINE_MS);
  const controlSection = document.getElementById('control-section');
  if (online) {
    controlSection?.classList.remove('disabled');
    showOfflineModal(false);
  } else {
    controlSection?.classList.add('disabled');
...
function refreshData(){ showToast('Refresh','Memperbarui data...', 'info', 1200); }
function exportData(){
  if (!currentUser) { showToast('Akses Ditolak','Silakan login dulu','warning'); return; }
...
function showOfflineModal(show){
  ...
  if(!m) return; m.style.display = show ? 'block' : 'none';
}
